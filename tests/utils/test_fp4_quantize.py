...
def _ref_fp4_quant_four_over_six_per_token(
    x: torch.Tensor,
) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    """Reference per-token NVFP4 4/6 quantizer with linear scale bytes."""
    if x.ndim != 2:
    ...
    e4m3_four_over_six_max = torch.tensor(256.0, device=x.device, dtype=torch.float32)
    max_float32 = torch.tensor(
        torch.finfo(torch.float32).max,
        device=x.device,
        dtype=torch.float32,
    ...
    global_encode_scale = torch.where(
        row_amax == 0.0,
        max_float32,
        torch.div(e4m3_four_over_six_max * e2m1_max, row_amax),
    )
...
def _make_nvfp4_per_token_test_input(
    dtype: torch.dtype,
    shape: tuple[int, int],
    init_data: str,
) -> torch.Tensor:
...
def test_nvfp4_per_token_quantize_te_reference(
    ...
    x = _make_nvfp4_per_token_test_input(dtype, shape, init_data)
...
@pytest.mark.parametrize("device", CUDA_DEVICES)
@torch.inference_mode()
def test_nvfp4_per_token_quantize_four_over_six_reference(
    dtype: torch.dtype,
    shape: tuple[int, int],
    sf_layout: SfLayout,
    init_data: str,
    ...
    """Per-token NVFP4 4/6 mode should match the MSE candidate reference."""
    if not _is_fp4_supported(torch.device(device)):
        pytest.skip("Nvfp4 Requires compute capability >= 10 and CUDA >= 12.8")

    torch.set_default_device(device)
    torch.manual_seed(42)
    monkeypatch.setenv("FLASHINFER_NVFP4_FOUR_OVER_SIX", "1")

    x = _make_nvfp4_per_token_test_input(dtype, shape, init_data)

    q_ref, scale_ref, per_token_scale_ref, _ = _ref_fp4_quant_four_over_six_per_token(x)
    expected_scale = _te_ref_scale_bytes_for_layout(scale_ref, sf_layout)
...
@pytest.mark.parametrize("device", CUDA_DEVICES)
@torch.inference_mode()
def test_nvfp4_per_token_quantize_four_over_six_candidate_selection(
    dtype: torch.dtype,
    sf_layout: SfLayout,
    device: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Per-token NVFP4 4/6 mode should exercise both MSE candidates."""
    if not _is_fp4_supported(torch.device(device)):
        pytest.skip("Nvfp4 Requires compute capability >= 10 and CUDA >= 12.8")
    ...
    q_ref, scale_ref, per_token_scale_ref, pick_four_ref = (
        _ref_fp4_quant_four_over_six_per_token(x)
    )
