...
3. On Mega, compare live-sized capacity with 32768 tokens/rank using separate tactic caches. Compare automatic tactics and, if needed, one fixed tactic on both capacities. Do not mix cold compile/tuning with steady-state samples.
...
### GPU correctness gate (not yet executed)
